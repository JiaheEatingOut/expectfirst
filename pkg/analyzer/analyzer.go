...
func run(pass *analysis.Pass) (interface{}, error) {
	...
	inspector.Preorder(nodeFilter, func(node ast.Node) {
		...
		if ok && isActualLike(secondArg.Name) {
			pass.Reportf(node.Pos(), "the actual-like variable name must be placed after the expect-like variable name")
			return
		}

		thirdArg, ok := args[2].(*ast.Ident)
		if ok && isExpectLike(thirdArg) {
			pass.Reportf(node.Pos(), "the expected-like variable name must be placed before the actual-like variable name")
			return
		}
	})
